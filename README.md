...
The application integrates with the Open Food Facts API to Read and display nutritional data. The data is also used to `Create` backend data which is managed on Airtable. Through the integration with the [Open Food Facts API](https://openfoodfacts.github.io/openfoodfacts-server/api/) and [Airtable](https://www.airtable.com/), the application is able to perform `CRUD` operations. (specifically `Create`, `Read` and `Delete`)
...
## MVP
Must include:
1. **5 Components:** The app shall have at least 5 components. Such as NavBar, SearchPage, FoodCard, TrackerPage, GoalDisplay.
2. **State & Props:** The app shall use useState for Search Results and Tracker and use Props to pass data to FoodCard.
3. **Lifting State:** As App.js will be the parent to all the components, lifting state shall be used for components to pass data between one another.
4. **Routing:** The app shall use React Router for routing.
...
As a user:
- I want to be able to change my calorie or macronutrient goals to update it based on my changing fitness goals.
